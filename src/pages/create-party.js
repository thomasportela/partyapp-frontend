...
var dateInput, changeDate1Input, changeDate2Input, changeDate3Input;

yup.addMethod(yup.string, 'date', function(){
  return this.test({name: 'date',exclusive: false, message: 'Acho que errou a data', test: function(){
    if(dateInput.props.value.length > 4 && dateInput.isValid()){
      return true
    }else{
      return false
    }
...
yup.addMethod(yup.string, 'date1', function(){
  return this.test({name: 'date1',exclusive: false, message: 'Data errada', test: function(){
    if(changeDate1Input.props.value.length > 4 && changeDate1Input.isValid()){
      return true
    }else{
      return false
    }
...
yup.addMethod(yup.string, 'date2', function(){
  return this.test({name: 'date2',exclusive: false, message: 'Data errada', test: function(){
    if(changeDate2Input.props.value.length > 4 && changeDate2Input.isValid()){
      return true
    }else{
      return false
    }
...
yup.addMethod(yup.string, 'date3', function(){
  return this.test({name: 'date3',exclusive: false, message: 'Data errada', test: function(){
    if(changeDate3Input.props.value.length > 4 && changeDate3Input.isValid()){
      return true
    }else{
      return false
    }
...
yup.addMethod(yup.string, 'time', function(){
  return this.test({name: 'time',exclusive: false, message: 'Errou a hora', test: function(value){
    if(value !== undefined){
      if(value.length > 4 && value.split(':')[0] < 24 && value.split(':')[1] < 60){
        return true
      }else{
        return false
      }
...
yup.addMethod(yup.string, 'price', function(){
  return this.test({name: 'price',exclusive: false, message: 'Erou', test: function(value){
    if(value !== undefined){
      if(isNaN(Number(value.replace(',','.'))) || Number(value.replace(',','.')) < 1){
        return false
      }else{
        return true
      }
...
export default function CreatePartyScreen({navigation}) {
  ...
  const {user} = useAuth()
  const {stopCreatingParty, tryingCancel , setTryingCancel} = useCreateParty()
  ...
  const [changeLoteByDate, setChangeLoteByDate] = useState(false)
  ...
  const changeLoteRules = (yup) => {
    if(lotes === 2 && changeLoteByDate){
      return {
        change_lote_date1: yup.string().required('Data vazia').date1(),
    ...
    if(lotes === 3 && changeLoteByDate){
      return {
        change_lote_date1: yup.string().required('Data vazia').date1(),
        change_lote_time1: yup.string().required('Hora vazia').time(),
        change_lote_date2: yup.string().required('Data vazia').date2(),
    ...
    if(lotes === 4 && changeLoteByDate){
      return {
        ...
        change_lote_date2: yup.string().required('Data vazia').date2(),
        change_lote_time2: yup.string().required('Hora vazia').time(),
        change_lote_date3: yup.string().required('Data vazia').date3(),
  ...
  const validationSchema = yup.object({
    ...
    date: yup.string().required('Tem que por a data da festa').date(),
  ...
  const PartyBanner = () => {
    if(banner === null){return <Icon name="plus" color={redOwned} size={45} />}
    else{
  ...
  function handleSubmit(values) {
    ...
    console.log(values)
    navigation.navigate('SelectLocation', {party: {...values, lotes, changeLoteByDate, obs1, obs2, obs3, banner}})
  }
        ...
        {(props) => (
          <View style={styles.formContainer}>
            ...
            <TouchableHighlight
              ...
              <Text style={styles.submitText}>TUDO CERTO!</Text>
            </TouchableHighlight>
