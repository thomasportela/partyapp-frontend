...
import { insertOrganizers } from '../services/createParty'
import showProfilePicture from '../helper/helperDefaults'
  
const initialLayout = { width: Dimensions.get('window').width };


export default function ChangeOrganizersScreen({route, navigation}) {
    const {_organizers, pid} = route.params;
    const {doService} = useAuth();
    const [people, setPeople] = useState([]);
    const [friends, setFriends] = useState([]);
    const [organizers, setOrganizers] = useState(_organizers);
    ...
    async function handleUpdateOrganizers(){
      try {
        const response = await doService(() => insertOrganizers(pid, organizers.map(organizer => organizer.uid)))
    ...
    const SubmitButton = () => {
        if(organizers.length === 0){
            return (
                <TouchableOpacity style={styles.submitButton} onPress={handleUpdateOrganizers}>
